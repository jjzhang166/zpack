...
void Package::closeFile(IWriteFile* file)
{
	...
	{
		delete static_cast<ZpWriteCompressFile*>(file);
	}
}
	...
	u32* outPackSize, u32* outFlag, u32 chunkSize)
{
	...
	if (fileSize != 0)
	{
		...
		fread(&tempBuffer[0], fileSize, 1, file);
		fclose(file);
	}
	zp::IWriteFile* writeFile = createFile(filename, fileSize, fileSize, chunkSize, flag);
	if (fileSize != 0)
	{
		if(writeFile->write(&tempBuffer[0], fileSize) == 0)
		{
			return false;
		}
	}
	closeFile(writeFile);
	int insertedIndex = getFileIndex(filename);
	if (insertedIndex >= 0)
	{
		FileEntry& dstEntry = getFileEntry(insertedIndex);
		if (fileSize == 0)
		{
			dstEntry.flag &= (~FILE_COMPRESS);
		}
		//temp
...
bool Package::addFileStream(const Char* filename, const Char* externalFilename, u32 fileSize, u32 flag,
						u32* outPackSize, u32* outFlag, u32 chunkSize)
{
	SCOPE_LOCK;
...
bool Package::addFileBuf(const Char* filename, const u8* fileBuf, u32 fileSize, u32 flag,
	u32* outPackSize, u32* outFlag, u32 chunkSize)
{
	std::vector<zp::u8> tempBuffer;
	tempBuffer.resize(fileSize);
	memcpy(&tempBuffer[0], fileBuf, fileSize);
	zp::IWriteFile* writeFile = createFile(filename, fileSize, fileSize, chunkSize, flag);
	if(writeFile->write(&tempBuffer[0], fileSize) == 0)
	{
		return false;
								...
								u32 flag, u64 contentHash)
{
	...
	return new ZpWriteCompressFile(this, entry.byteOffset, entry.originSize, entry.chunkSize, entry.flag, entry.nameHash);
}
...
bool Package::readHeader()
{
	...
	u64 iFileEntry = sizeof(FileEntry);
	//_fseeki64(m_stream, 0, SEEK_END);
	fseek(m_stream, 0, SEEK_END);
	u64 packageSize = ftell(m_stream);
	if (packageSize < iPackageHeader)
	{
		return false;
	}
	//_fseeki64(m_stream, 0, SEEK_SET);
	fseek(m_stream, 0, SEEK_SET);
	fread(&m_header, iPackageHeader, 1, m_stream);
	if (m_header.sign != PACKAGE_FILE_SIGN
		|| m_header.headerSize != iPackageHeader
...
u32 Package::insertFileEntry(FileEntry& entry, const Char* filename)
{
	...
	for (u32 fileIndex = 0; fileIndex < maxIndex; ++fileIndex)
	{
				...
				|| lastEnd >= m_header.filenameOffset + m_header.allFilenameSize))
		{
			...
			FileEntry& newEntry = getFileEntry(fileIndex);
			newEntry = entry;
...
void Package::setResFormat( u8 resFormat)
{
	m_header.resFormat = resFormat;
	m_dirty = true;
}

zp::u8 Package::getResFormat() const
{
	return m_header.resFormat;
}
